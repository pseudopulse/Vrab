            ...
            if (stopwatch >= searchDelay) {
                ...
                if (target) {
                    if (lastTarget != target) {
                        new SyncVrabTarget(base.gameObject, target).Send(R2API.Networking.NetworkDestination.Server);
                        new SyncVrabTarget(base.gameObject, target).Send(R2API.Networking.NetworkDestination.Clients);
                    }
                    lastTarget = target;
